...
__global__ void flags_kernel(unsigned int * barrierBuffers,
                             ...
                             const float cutoffVal,
                             const unsigned int numIters/*,
                             const region_t globLocksReg, // for barrierBuffers
                             const region_t localLocksReg, // for perSMBarrierBuffers
                             const region_t flagsReg,
                             const region_t outReg,
                             const region_t randReg*/) {
  // local variables
  ...
  */
  /*
  if (isMasterThread) {
    __denovo_setAcquireRegion(flagsReg);
    __denovo_addAcquireRegion(outReg);
    __denovo_addAcquireRegion(globLocksReg);
    __denovo_addAcquireRegion(localLocksReg);
    __denovo_addAcquireRegion(randReg);
  }
  __syncthreads();
  */
  ...
  */
    ...
    workerTBs(stop, dirty, barrierBuffers, perSMBarrierBuffers, randArr,
              ...
              perSM_blockID, numTBs_perSM, arrayStride, cutoffVal, numIters);
  }

  /*
  if (isMasterThread) {
    __denovo_gpuEpilogue(flagsReg);      // written with atomics
    __denovo_gpuEpilogue(outReg);        // written with data stores
    __denovo_gpuEpilogue(globLocksReg);  // written in barrier function
    __denovo_gpuEpilogue(localLocksReg); // also written in barrier function
    __denovo_gpuEpilogue(randReg);  // read-only by workers
  }
  */
}
