                                 ...
                                 const int maxBlocks)
{
  ...
  /*
    Each SM has maxBlocks*2 locations in perSMBarrierBuffers, so my SM's
    inVars locations start at perSMBarrierBuffers[smID*2*maxBlocks] and my
    SM's outVars locations start at
    perSMBarrierBuffers[smID*2*maxBlocks + maxBlocks].
  */
  unsigned int * const inVars  = perSMBarrierBuffers + (maxBlocks * smID * 2);
  unsigned int * const outVars = perSMBarrierBuffers + ((maxBlocks * smID * 2) + maxBlocks);
