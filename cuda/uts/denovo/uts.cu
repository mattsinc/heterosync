...
// need these for integrating with simulator (BEGIN/END_SIMULATION, etc.)
#include <treebar.h>
...
TreeBar barrier;                   // tree barrier for synchronization
...
void callGPU()
{
  ...
  uts_tree = (uts_node*) malloc(sizeof(uts_node) * num_nodes); 
  cudaError_t cudaErr = cudaGetLastError();
#ifdef DISCRETE_GPU
  checkError(cudaErr, "Before cudaMalloc");
  cudaMalloc((void**)&d_uts_tree, sizeof(uts_node) * num_nodes); 
  cudaErr = cudaGetLastError();
  checkError(cudaErr, "After cudaMalloc1");
#endif
  ...
  work_stacks = (int*) malloc(sizeof(int) * (global_stack_size + local_stack_size * cores_used));
#ifdef DISCRETE_GPU
  cudaMalloc(&d_work_stacks, sizeof(int) * (global_stack_size + local_stack_size * cores_used));
  cudaErr = cudaGetLastError();
  checkError(cudaErr, "After cudaMalloc2");
#endif
  stack_heads = (int*) malloc(sizeof(int) * (cores_used+1));
#ifdef DISCRETE_GPU
  cudaMalloc(&d_stack_heads, sizeof(int*) * (cores_used+1));
  cudaErr = cudaGetLastError();
  checkError(cudaErr, "After cudaMalloc3");
#endif
  for(int i=0; i<cores_used+1; i++) {
    stack_heads[i] = 0;
  }
  stack_locks = (unsigned int*) malloc(sizeof(int) * (cores_used+1));
#ifdef DISCRETE_GPU
  cudaMalloc(&d_stack_locks, sizeof(int)*(cores_used+1));
  cudaErr = cudaGetLastError();
  checkError(cudaErr, "After cudaMalloc4");
#endif
  for(int i=0; i<cores_used+1; i++) {
    stack_locks[i] = 0;
  }
  ...
  while(init_nodes.size() < num_init_nodes) {
    ...
    if (init_nodes.size() == 0) {
      printf("Error: not enough nodes to fill stacks (need frontier of size %d, there are %d leaf nodes)\n", num_init_nodes, leaf_nodes.size());
      assert(0);
    }
...
void *doWork(void *thread_id) {
  ...
  // wait for everyone before starting the simulation
  TreeBarrier(&barrier, tid);
...
#ifdef DISCRETE_GPU
    cudaError_t cudaErr = cudaGetLastError();
    checkError(cudaErr, "Before cudaMallocs");
    cudaMalloc(&d_nodes_processed, sizeof(unsigned int));
    cudaMalloc(&d_leaves_found, sizeof(int));
    cudaMalloc(&d_leaf_sum, sizeof(unsigned int));
    cudaErr = cudaGetLastError();
    checkError(cudaErr, "After cudaMallocs");
...
#endif
    //printf("lock addr:%p val:%d\n", stack_lock_ptr, stack_locks[0]);
    //printf("head addr:%p val:%d\n", stack_head_ptr, stack_heads[0]);
    //printf("nodes processed addr:%p val:%d\n", nodes_processed_ptr, nodes_processed);
    //printf("leaves found addr:%p val:%d\n", leaves_found_ptr, leaves_found);
    //printf("leaf sum addr:%p val:%d\n", leaf_sum_ptr, leaf_sum);
    for(int i=0; i<cores_used; i++) {
      //printf("core %d local head:%d stack:", i, stack_heads[i+1]);
      //for(int j=0; j<stack_heads[i+1]; j++) {
      //  printf("%d ", work_stacks[global_stack_size+local_stack_size*i+j]);
      //}
      //printf("\n");
    }
...
int main(int argc, char *argv[])
{
  ...
  TreeBarInit(&barrier, __num_real_threads__);
  callGPU();
